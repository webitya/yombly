...
export default function AdminCaseStudiesClient() {
  const { data, mutate } = useSWR("/api/case-studies", fetcher)
  const items = data?.items || []

  return (
    <div className="space-y-6">
      <AdminCaseStudyForm onCreated={() => mutate()} />
      <section>
        <h2 className="text-xl font-semibold mb-3">Existing Case Studies</h2>
        <div className="grid gap-3 md:grid-cols-2">
          ...
          {!items.length && <p className="text-muted-foreground">No case studies yet.</p>}
        </div>
      </section>
    </div>
  )
