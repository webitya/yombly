...
export default function CaseStudyList() {
  ...
  if (isLoading)
    return <p className="text-sm text-gray-500">Loading case studies...</p>;
  if (error)
    return <p className="text-sm text-red-500">Failed to load case studies.</p>;

  const items = data?.items || [];
  if (!items.length)
    return <p className="text-sm text-gray-500">No case studies available.</p>;
